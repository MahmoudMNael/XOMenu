//
// Created by Mohamed Fathy on 12/16/2023.
//
// Class definition for XO_GameManager class
// Author:  Mohammad El-Ramly
// Date:    10/10/2022
// Version: 1

#include <iostream>
#include"BoardGame_Classes.hpp"
using namespace std;

GameManager_5x5::GameManager_5x5(X_O_Board_5x5* bPtr, Player* playerPtr[2] ) {
    boardPtr = bPtr;
    players[0] = playerPtr[0];
    players[1] = playerPtr[1];

}

void GameManager_5x5::run() {
    int x, y;
    boardPtr->display_board();

    while (!boardPtr->game_is_over()) {
        for (int i:{0,1}) {
            players[i]->get_move(x, y);
            while (!boardPtr->update_board (x, y, players[i]->get_symbol())){
                players[i]->get_move(x, y);
            }
            boardPtr->display_board();
            if (boardPtr->is_winner5x5()>0){
                if( boardPtr->is_winner5x5()==1){
                    cout  << players[0]->to_string() << " wins\n";
                }else if(boardPtr->is_winner5x5()==2){
                    cout  << players[1]->to_string() << " wins\n";
                }

                return;
            }
            if (boardPtr->is_draw()){
                cout << "Draw!\n";
                return;
            }
        }
    }
}
